...
| 2 | RocksDB's static lib does not push gcs_server over its memory budget (PLAN pivot trigger: 50 MB). | 2 | `gcs_server` with RocksDB = 32 MB unstripped / 24 MB stripped. Well under 50 MB. | **Verified** |
...
| R3 | Binary size pushes head pod over budget | **Partially closed** (Phase 3, 8) — 24 MB stripped, well under 50 MB pivot trigger. Memory at runtime pending Phase 7 v2. |
