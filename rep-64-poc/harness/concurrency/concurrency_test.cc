...
//   1. GetNextJobID is atomic and produces no duplicates / no skips
//      under N-thread contention (REP "Option 1: Read-modify-write
//      under a mutex").
//   2. AsyncPut from multiple producer threads against a single
//      io_context yields fully-recoverable, value-correct state.
//
// Phase 5 uses the mutex-RMW path (already implemented in Phase 3).
...
//
// These tests deliberately live outside the upstream-bound test target
// (//:rocksdb_store_client_test) so that the latter stays small and
// purpose-specific. The Bazel target //rep-64-poc/harness/concurrency
// :concurrency_test is the POC's stress harness; running it under
// --config=tsan / --config=asan-clang on a properly-tooled host is the
// definitive concurrency check.
...
// --- 1. GetNextJobID: no duplicates across threads. ----------------------
// Spawns kThreads threads each calling GetNextJobID kPerThread times;
// asserts the union is exactly the integer range [1, kThreads*kPerThread]
// with no duplicates and no gaps. This is the cleanest possible signal
// that the mutex-RMW path is correctly atomic + monotonic.

TEST_F(RocksDbConcurrencyTest, GetNextJobIDNoDuplicatesAcrossThreads) {
  ...
  for (int t = 0; t < kThreads; ++t) {
    workers.emplace_back([this, &collector_mutex, &all_ids] {
      ...
      for (int i = 0; i < kPerThread; ++i) {
        local.push_back(client_->GetNextJobID());
      }
...
// --- 2. GetNextJobID: monotonic per thread. ------------------------------
// Even under contention, the IDs a single thread receives must be in
// strictly increasing order. (The values may have arbitrary gaps —
// other threads got the IDs in between — but each thread's own
// sequence must monotonically increase.)

TEST_F(RocksDbConcurrencyTest, GetNextJobIDMonotonicPerThread) {
  ...
  for (int t = 0; t < kThreads; ++t) {
    workers.emplace_back([this, t, &per_thread] {
      ...
      for (int i = 0; i < kPerThread; ++i) {
        v.push_back(client_->GetNextJobID());
      }
    });
  }
  for (auto &w : workers) w.join();
...
// Multiple producers issuing AsyncPut concurrently against a single
// io_context. After all callbacks fire, every key written must be
// readable with the value the producer wrote — no torn writes, no
// dropped puts, no value mix-ups across keys.
  ...
  for (int t = 0; t < kThreads; ++t) {
    producers.emplace_back([this, t, kTable, &put_acks] {
      for (int i = 0; i < kPerThread; ++i) {
        ...
        Status s = client_->AsyncPut(kTable, key, value, /*overwrite=*/true,
                                     [&put_acks](bool) {
                                       put_acks.fetch_add(1);
                                     });
...
// --- 4. Job ID counter durability under load + restart. ------------------
// After a heavy concurrent burst, close the client and reopen at the
// same path. The next ID must be strictly greater than the largest
// ID issued in the previous lifetime — no roll-back, no reuse.
